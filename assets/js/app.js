...
console.log("Serach Filed ", searchField);
      ...
      fetchData(URL.geo(searchField.value), function (locations) {
        ...
        for (const { name, lat, lon, country, state } of locations) {
          ...
          searchItem.innerHTML = `
                        ...
                        <a href="#/weather?lat=${lat}&lon=${lon}" class="item-link has-state" aria-labl="${name} weather" data-search-toggler></a>
                    `;
          searchResult
...
export const updateWeather = function (lat, lon) {
  ...
  if (window.location.hash == "#/current-location") {
    ...
    currentLocationBtn.removeAttribute("");
  }
  ...
  fetchData(URL.currentWeather(lat, lon), (currentWeather) => {
    ...
    fetchData(URL.airPollution(lat, lon), function (airPollution) {
      ...
      card.innerHTML = `
            <h2 class="title-2" id="highlight-label">Todays
                Highlight</h2>
      ...
      for (let i = 7, len = forecastlist.length; i < len; i += 8) {
        ...
        li.innerHTML = `
          ...
          module.monthNames[date.getUTCMonth()]
            ...
            document.querySelector("[data-forecast-list]").appendChild(li);
      }
